...
<ul>
<li>token - Получаемый ключ с YooKassa как альтернатива платежным данным.</li>
<li>IdempotenceKey - Уникальный ключ который генерируется на стороне клиента при старте приложения и обновляется при получении статуса произведенного платежа. Ключ исключает создание дубликатов платежей при возникновении ошибок различного рода.</li>
</ul>
